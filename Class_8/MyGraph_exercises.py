    ...
    def reachable_bfs(self, v):
        ...
        while len(l) > 0:  # enquanto ha elementos na lista l (lista de queue)
            ...
            for elem in self.graph[node]:  # correr os nos no node a pesquisar
                nwnode, wg = elem
                if nwnode != v:  # se o v não entra na lista de nos acessiveis e for diferente é adiciona a lista
                    res.append(nwnode)  # controi o resultado sempre a colocar no fim, ou seja os primeiros ficam para ultimo
        ...
        if s == d:
            ...
            while len(l) > 0:  # enquanto ha elementos na lista l (lista de queue)
                ...
                if nxnode not in vis and nxnode not in l and nxnode != node:  # adicionar á queue
                    l.append((nxnode, [(node, nxnode)], swg + bwg))
                    vis.append(node)
