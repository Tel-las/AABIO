...
class MetabolicNetwork (MyGraph):
    ...
    def convert_metabolite_net(self, gmr):
        for m in gmr.node_types["metabolite"]:
            self.add_vertex(m)
            reactions = gmr.get_successors(m)
            for reaction in reactions:
                metabolites = gmr.get_successors(reaction)
            for metabolite in metabolites:
                if m != metabolite:
                    self.add_edge(m, metabolite)
        
    def convert_reaction_graph(self, gmr):
        for r in gmr.node_types["reaction"]:
            self.add_vertex(r)
            metabolites = gmr.get_successors(r)
            for metabolite in metabolites:
                reactions = gmr.get_successors(metabolite)
                for reaction in reactions:
                    if r != reaction:
                        self.add_edge(r, reaction)
...
def teste3():
    ...
    #rrsn.print_graph()
    print()
    mean_degree = mrn.mean_degree("out")
